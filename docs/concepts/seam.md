...
spec:
  ...
  emits:
    - productRef: telco.gold.payment_recovery_moment   # id of a .fluid.yml in the bundle
      ...
      fluidVersion: "0.7.5"                            # pinned, vendored FLUID version
```
...
- **Version skew** — `fluidVersion` is an enum of exactly the FLUID schema
  versions vendored in this release (`0.7.3`–`0.7.5`). A contract can't
  silently target a FLUID version the toolchain has never seen.
...
3. asserts the pinned version equals the document's own `fluidVersion`;
4. asserts `exposeId` is a member of the product's `exposes[]`;
5. **consent strictness** — the emitted expose's `policy.agentPolicy` must be
